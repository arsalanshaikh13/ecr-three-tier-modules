...
# This replaces the empty bind mount block
  volume {
    ...
    efs_volume_configuration {
      file_system_id     = var.backend_data_id
      root_directory     = "/"
  ...
  container_definitions = jsonencode([
    {
      ...
      healthCheck = {
        command     = ["CMD-SHELL", "wget --no-verbose --tries=3 --spider http://127.0.0.1:${backend_tg_port}/${var.backend_health_check_path} || exit 1"]
        interval    = 10
        timeout     = 5
        retries     = 5
      }
  ...
  container_definitions = jsonencode([
    {
      ...
      healthCheck = {
        ...
        command     = ["CMD-SHELL", "wget --no-verbose --tries=3 --spider http://127.0.0.1:${frontend_tg_port}/${var.frontend_health_check_path} || exit 1"]
        interval    = 30
        timeout     = 10
        retries     = 3
        startPeriod = 75
